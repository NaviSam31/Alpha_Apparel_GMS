...
const DeleteMaterialRequirement = () => {
  ...
  const handleDeleteDesign = () => {
    setLoading(true);
    axios.delete(`http://localhost:5555/materialRequirement/${id}`)
      .then(() => {
        setLoading(false);
        navigate('/materialRequirement/home');
      })
      .catch((error) => {
        setLoading(false);
    ...
    <div className="main-content">
      ...
      <div className='button-container'>
        ...
        <div className='main-content p-4'>
          <BackButton />
          <h1 className='text-3xl my-4'>Delete Material Requirement Record</h1>
          {loading ? <Spinner /> : ''}
          <div className='flex flex-col items-center border-2 border-sky-400 rounded-xl w-[600px] p-8 mx-auto'>
            <h3 className='text-2xl'>Are you sure you want to delete this material requirement record?</h3>
            <button className='p-4 bg-red-600 text-white m-8 w-full' onClick={handleDeleteDesign}>
              Yes, Delete it.
            </button>
          </div>
        </div>
...
export default DeleteMaterialRequirement;
